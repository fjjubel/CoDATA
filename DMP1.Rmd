...
- This project will result in the production of a relational spatially-enabled database integrating all known surface water, ground
water, and water quality data for the middle Rio Grande basin study area. Additionally, Visual Basic for Applications (VBA) code and
Structured Query Language (SQL) code are products of the project. 
- All updateable datasets are acquired from the original data source (for example, EPA websites). Updatable data sources are acquired at specified.
intervals – quarterly, or as needed. As new static data sources are discovered, they will be integrated into the proposed compendium. 
...
1. (http://www.esri.com/software/arcgis/index.html), a Geographic
...
identified dataset. 
    a. Quality control of the database will be performed
...
to manage possible data duplicates. 
    a. Potential duplicate site IDs, will
...
The dataset has `r nrow(usgs)` samples sites 
### sub heading
- A data dictionary will be created that defines the table definition,
    a. table fields, and table field data types. An entity-relationship diagram
    a. will be created that defines the relational structure of the database. A
...
```{r plot, echo=TRUE}
plot(faithful, main = "Do I wait longer eruptions?")
...
with University/Participating institutional and NSF policies, @Briney_2015. Depending
